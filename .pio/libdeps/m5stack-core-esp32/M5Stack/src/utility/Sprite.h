...
class TFT_eSprite : public TFT_eSPI {
 public:
  ...
  void drawChar(int32_t x, int32_t y, uint16_t c, uint32_t color, uint32_t bg, uint8_t size),
      ...
      fillScreen(uint32_t color), fillSprite(uint32_t color),

      // Define a window to push 16 bit colour pixels into in a raster order
      // Colours are converted to 8 bit if depth is set to 8
      setWindow(int32_t x0, int32_t y0, int32_t x1, int32_t y1), pushColor(uint32_t color),
  ...
  void getRotatedBounds(float sina, float cosa, int16_t w, int16_t h, int16_t xp, int16_t yp, int16_t *min_x,
